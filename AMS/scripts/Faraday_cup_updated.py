...
for i in range(0, len(data_info), 1):
    ...
    data_peaks_found.append(find_peaks(np.abs(data_loaded[i][1]), height = data_info[i][4], distance = min_dist)[0])
    data_peaks_values.append([])
# peak correction
    for j in range(0, len(data_peaks_found[i]), 1): # for each found peak
        left_idx = data_peaks_found[i][j]
        right_idx = data_peaks_found[i][j]
        for l in range(1, min_dist+1, 1): # shift up to peak width to left
            if (data_peaks_found[i][j] - l >= 0):
                if np.abs(data_loaded[i][1][data_peaks_found[i][j] - l]) >= data_info[i][4]:
                    left_idx = data_peaks_found[i][j] - l
                else:
                    break
            else:
                break
        ...
        for l in range(1, min_dist+1, 1): # shift up to peak width to right
            if (data_peaks_found[i][j] + l < len(data_loaded[i][1])):
                if np.abs(data_loaded[i][1][data_peaks_found[i][j] + l]) >= data_info[i][4]:
                    right_idx = data_peaks_found[i][j] + l
                else:
                    break
            else:
                break
        data_peaks_found[i][j] = (np.sum(np.arange(left_idx, right_idx+1, 1) * data_loaded[i][1, left_idx:right_idx+1]) / np.sum(data_loaded[i][1, left_idx:right_idx+1])) + 1
...
for i in range(0, len(plot_nums), 1):
    ...
    peak_plot_y = np.where(np.abs(data_loaded[plot_nums[i]][1][data_peaks_found[plot_nums[i]]]) < np.max(data_filtered[plot_nums[i]][1]), data_loaded[plot_nums[i]][1][data_peaks_found[plot_nums[i]]], np.max(data_filtered[plot_nums[i]][1]))
    ...
    for j in range(0, len(data_peaks_found[plot_nums[i]]), 1):                    # peak labels
        ax.text(mass_calib_LE(data_loaded[plot_nums[i]][0][data_peaks_found[plot_nums[i]]][j]) + label_x_offset,
                peak_plot_y[j],
...
fig.subplots_adjust(bottom=0.07, top=0.95, left=0.07, right=0.98)
plt.show()
